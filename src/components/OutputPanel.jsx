...
const DownloadButtons = ({ params }) => {
  ...
  const commonFileName =
    "Disc-D" +
    (Number.isInteger(Math.round(params.fixedRingDiameter * 1000) / 1000)
      ? params.fixedRingDiameter.toFixed(0)
      : params.fixedRingDiameter.toFixed(3)
    )
      .toString()
      .replace(".", "_") +
    "-E" +
    (Number.isInteger(Math.round(params.eccentricity * 1000) / 1000)
      ? params.eccentricity.toFixed(0)
      : params.eccentricity.toFixed(3)
    )
      .toString()
      .replace(".", "_") +
    "-Dp" +
    (Number.isInteger(Math.round(params.externalPinDiameter * 1000) / 1000)
      ? params.externalPinDiameter.toFixed(0)
      : params.externalPinDiameter.toFixed(3)
    )
      .toString()
      .replace(".", "_") +
    "-N" +
    (Number.isInteger(Math.round(params.numberOfExternalPins * 1000) / 1000)
      ? params.numberOfExternalPins.toFixed(0)
      : params.numberOfExternalPins.toFixed(3)
    )
      .toString()
      .replace(".", "_") + 
      "-Phase" + params.phase;
  ...
  const downloadDXF = (disk) => {
    const fileName = `${commonFileName}-Disc${disk}.dxf`;
  ...
  const downloadSTL = async (disk) => {
    const fileName = `${commonFileName}-Disc${disk}.stl`;
    setLoading("stl", true);
    try {
  ...
  const downloadSTEP = async (disk) => {
    const fileName = `${commonFileName}-Disc${disk}.step`;
    ...
    <>
      <div className="output-container">
        ...
        <span
          ...
          data-tooltip="Download model in DXF(2D) format."
        >
          ?
        </span>
      </div>
      <div className="output-container">
        ...
        <span
          className="info-icon tooltip"
          data-tooltip="Download model in STL(3D) format."
        >
          ?
        </span>
      </div>
      <div className="output-container">
        ...
        <span
          className="info-icon tooltip"
          data-tooltip="Download model in STEP(3D) format."
        >
          ?
        </span>
      </div>
